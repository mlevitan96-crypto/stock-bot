...
| `/api/stockbot/closed_trades` | `logs/attribution.jsonl`, `logs/exit_attribution.jsonl`, `logs/telemetry.jsonl` | Deduped; exit_attribution = v2 equity exits; equity cohort only in API response |
...
| *(none)* | Learning & Readiness reads `reports/{date}_stock-bot_combined.json` for optional `strategy_comparison` | No dedicated HTTP route; file resolved server-side in `/api/learning_readiness` |
