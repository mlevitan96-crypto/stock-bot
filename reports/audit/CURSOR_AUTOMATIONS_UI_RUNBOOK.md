...
```
You are the Weekly Governance Summary automation for the stock-bot repository. Trigger: Sunday 00:00 UTC (cron 0 0 * * 0). Run on main branch.

1. Use current date or last Sunday for the report filename: reports/board/WEEKLY_GOVERNANCE_SUMMARY_<YYYY-MM-DD>.md

2. Gather for the past 7 days (from git history, branch state, repo files):
   a) COMMITS TO MAIN — Count and list/summarize commits. Use git log.
   b) PRs MERGED — If GitHub API/PR context available: count and list merged PRs. Otherwise note "Manual check: GitHub PR list, merged in last 7 days."
   c) CSA VERDICTS — List reports/audit/CSA_VERDICT_*.json or CSA_VERDICT_LATEST.json; summarize verdict and mission_id.
   d) SRE ANOMALIES — reports/audit/SRE_STATUS.json or similar; summarize. Else "No SRE status file found."
   e) DEPLOYS — reports/audit/: DEPLOYMENT_PROOF, DEPLOY_*, B2_* proof files updated in last 7 days. One-line per artifact.
   f) SHADOW/PAPER/LIVE CHANGES — From reports/audit/ and reports/board/. Summarize.
   g) CONFIG CHANGES — Notable config or env changes in main in last 7 days. Brief list or "None detected."

3. Write a single Markdown report to reports/board/WEEKLY_GOVERNANCE_SUMMARY_<YYYY-MM-DD>.md:
   # Weekly Governance Summary — <YYYY-MM-DD>
   ## Commits to main / ## PRs merged / ## CSA verdicts / ## SRE anomalies / ## Deploys / ## Shadow / paper / live changes / ## Config changes
   ---
   *Generated by Cursor Automation (weekly_governance_summary).*

4. Do NOT send Slack (Slack is disabled). Do not modify droplet code or runtime. If the automation can commit, commit and push only this report file; otherwise write and note in run summary.
```
