...
# Version: 2026-04-08 (Alpaca V2 Harvester phase; telemetry milestone latching; §1.0)
...
## 1.0 Alpaca V2 Harvester phase (current — 2026-04-08)

- **Phase / status:** **Alpaca V2 Harvester** — paper (and scaled-capital) **data collection** with **V5.0 Passive Hunter** execution (midpoint-pegged limits, spread guard, 24/5 routing when eligible; see Implementation History **2026-04-03**).
- **Milestones:** Harvester era has **crossed 100 closed trades** with **100% execution-join coverage** on the audited strict cohort; next scale target is **250** trades for ML retrain readiness. Counts and export CSVs are **floored at `STRICT_EPOCH_START`** so pre-era rows do not pollute milestones.
- **Canonical logs:** `logs/exit_attribution.jsonl` (closed trades), `logs/run.jsonl`, `logs/signal_context.jsonl`, plus warehouse/replay outputs under `reports/` and `replay/` per §1.2.
- **Harvester exports (Gemini / board tooling):** `reports/Gemini/entries_and_exits.csv`, `reports/Gemini/signal_intelligence_spi.csv` — generated by **`scripts/extract_gemini_telemetry.py`** (Harvester-era rows only when extractor applies the strict epoch floor).
- **Milestone watcher (SRE):** **`scripts/telemetry_milestone_watcher.py`** — refreshes Gemini CSVs, counts trades since cutoff, SPI sanity at **10**, deduped Telegram for **100** and **250** (`MILESTONE_ALERT_THRESHOLDS`). Effective cutoff is **`max(optional TELEMETRY_MILESTONE_SINCE_DATE 00:00 UTC, STRICT_EPOCH_START)`**; **`data/.milestone_state.json`** stores sent flags; floor changes clear OOS keys so **250 cannot skip 100**. Typical droplet cron: hourly append to `/var/log/telemetry_milestone.log` (see script docstring).
...
2. **Secondary sink — `logs/scoring_flow.jsonl`:** Time-series **`composite_calculated`** lines (per symbol) hold **UW/composite component breakdown** (flow, dark_pool, greeks_gamma, etc.). ML feature extraction **does not** key on `trade_id` in that file; it joins by **`symbol` / `symbol_normalized`** and **entry time**.

3. **Wide join policy (scoring_flow → training CSV):** ML feature extraction uses a **4-hour “Last Known Score” lookback** for `scoring_flow.jsonl` joins to maximize training feature density while keeping scores **contextually tied to the entry**. **`scripts/telemetry/alpaca_ml_flattener.py`** selects the **most recent** `composite_calculated` row for the trade symbol with **`ts <= entry_anchor`** and **`entry_anchor - ts <= 14400` seconds** (override via `--scoreflow-lookback-sec`). **Entry anchor** resolves in order: `entry_ts` on the exit row → `entry_timestamp` → open instant parsed from `trade_id` (`open_<SYM>_<ISO>`). Duplicate timestamps in `scoring_flow` for the same symbol are **coalesced** (last line wins). Exported fields include **`mlf_scoreflow_total_score`**, **`mlf_scoreflow_components_*`**, **`mlf_scoreflow_snapshot_ts_epoch`**, **`mlf_scoreflow_snapshot_age_sec`**, **`mlf_scoreflow_lookback_sec_applied`**.

4. **Harvester phase:** **ACTIVE.** Cohort filtering for strict-era learning uses **`STRICT_EPOCH_START`** in `telemetry/alpaca_strict_completeness_gate.py` (open time from `trade_id` / entry instant). **100-trade milestone data** and subsequent closes in that era remain **intact inside these payloads**; emptiness of `signal_context.jsonl` does **not** imply loss of ML features for that cohort.

5. **Training export:** **`scripts/telemetry/alpaca_ml_flattener.py`** writes **`reports/Gemini/alpaca_ml_cohort_flat.csv`** — flattened `mlf_*` feature columns plus base trade fields for XGBoost and similar tools. The script prints **`scoreflow_snapshot_coverage_pct`** and **`scoreflow_total_score_populated_pct`** after each run for SRE verification.

6. **Invariant (ML):** Do not assert “missing `signal_context`” as a missing-feature incident for Harvester without checking **`exit_attribution`** embeds and **`scoring_flow`** join coverage separately.
