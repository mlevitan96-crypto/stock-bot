...
- **Milestone + integrity cadence (root):** `alpaca-telegram-integrity.timer` + `scripts/run_alpaca_telegram_integrity_cycle.py` (see **Alpaca Telegram + data integrity cycle** below). Legacy `notify_alpaca_trade_milestones` crontab lines must be absent. **Full fast-lane 15m / 4h entries** may or may not be present — confirm with `crontab -l` and `scripts/install_fast_lane_cron_on_droplet.py` install state before assuming they are live.
...
- **Manual tests:** Run cycle and supervisor --force; confirm ledger/state and Telegram. See `docs/ALPACA_FAST_LANE_LIVE_ACTIVATION_VERIFICATION.md`. **Always check droplet with real data** (ledger, state, logs, cron, .alpaca_env) when verifying fast-lane or Telegram.
- **Dashboard:** Tab "Alpaca Fast-Lane 25-Trade PnL" (More dropdown); API `/api/stockbot/fast_lane_ledger`.
- **CSA/SRE:** Schema and directory layout approved; isolation confirmed (no main ledger or config writes). Full go-forward promotion design: `docs/SHADOW_25_TRADE_PROMOTION_EXPERIMENT_DESIGN.md`; CSA/SRE to choose experiment root path and refine angles.
- **Alpaca Fast-Lane 25-trade deep review (2026-03-17):** Board packet: `reports/ALPACA_FASTLANE_25_BOARD_REVIEW_20260317_1525.md` (10 cycles, 250 trades). CSA review: `reports/audit/CSA_REVIEW_ALPACA_FASTLANE_25_20260317_1525.md`. **CSA verdict: NO PROMOTION** — script candidate time_of_day:afternoon was relative best but still negative mean PnL; edge not sufficiently stable; revisit after more cycles. This batch is **GOVERNANCE-CLOSED**. No live or paper changes authorized by this review.
- **Alpaca 2000-trade edge discovery pipeline:** Script `scripts/alpaca_edge_2000_pipeline.py` — frozen dataset (up to 2000 trades from `logs/exit_attribution.jsonl`), bar-by-bar telemetry placeholder, baseline metrics, counterfactual studies (hold-duration, signal-score gating), stability checks, promotion shortlist, board packet, CSA/SRE review artifacts, Telegram (start + completion + verdict). Run on droplet for full data: `scripts/run_alpaca_edge_2000_on_droplet.py`. Output: `reports/alpaca_edge_2000_<TS>/` (TRADES_FROZEN.csv, INPUT_FREEZE.md, BASELINE_METRICS, studies/, STABILITY_CHECKS.md), `reports/ALPACA_EDGE_BOARD_REVIEW_<TS>.md`, `reports/ALPACA_EDGE_PROMOTION_SHORTLIST_<TS>.md`, `reports/audit/CSA_REVIEW_ALPACA_EDGE_<TS>.md`, `reports/audit/SRE_REVIEW_ALPACA_EDGE_<TS>.md`. **No live or paper changes.**
...
- **Purpose:** Single coherent Alpaca-only pipeline for (1) **250-trade milestone** Telegram (once per US regular-session anchor day, counting unique canonical `trade_key` closes since that session’s 09:30 ET open), (2) **data integrity / drift** Telegram when warehouse coverage, staleness, exit_attribution tail probes, strict completeness regression (ARMED→BLOCKED), or post-close / direction-readiness pager windows fail, (3) **safe self-heal** (mkdir log dirs; `systemctl try-restart` **alpaca-postclose-deepdive.service** only if failed — never `stock-bot`).
- **Entry:** `scripts/run_alpaca_telegram_integrity_cycle.py` — flags: `--dry-run`, `--skip-warehouse`, `--no-self-heal`, `--send-test-milestone`, `--send-test-integrity` (both prefix `[TEST]`). **systemd:** `deploy/systemd/alpaca-telegram-integrity.service` + `alpaca-telegram-integrity.timer` (every **10 minutes**). **Install:** `bash scripts/install_alpaca_telegram_integrity_on_droplet.sh` (copies units, `daemon-reload`, enable/start timer). Units load `EnvironmentFile` **both** `/root/stock-bot/.env` and `/root/.alpaca_env` (optional `-` prefix).
- **Config:** `config/alpaca_telegram_integrity.json` — `enabled`, `milestone_trade_count` (default 250), `warehouse_days_when_run`, `warehouse_run_every_n_cycles` (throttle full truth warehouse mission), `warehouse_coverage_file_max_age_hours`, `coverage_thresholds_pct`, `exit_attribution_tail_lines`, `integrity_alert_cooldown_sec`, `strict_regression_alert_cooldown_sec`, `self_heal` toggles. Full warehouse mission runs only during **US RTH ET** (throttled); milestone and strict probe run every invocation.
- **State:** `state/alpaca_milestone_250_state.json` (per-session-anchor idempotent milestone), `state/alpaca_telegram_integrity_cycle.json` (cycle counter, cooldowns, last_good snapshot). **Log:** `logs/alpaca_telegram_integrity.log` (append-only line per run; `telegram_failure_detector` milestone window prefers this file, then legacy `notify_milestones.log`).
...
- **Code layout:** `telemetry/alpaca_telegram_integrity/` (session open clock, milestone counting, warehouse summary parse, strict/exit probes, templates, self-heal). **Tests:** `tests/test_alpaca_telegram_integrity.py`. **No trading logic, signals, or Kraken paths.**
