#!/usr/bin/env python3
"""
Run entry intelligence parity audit on droplet and pull report to local.
Uses DropletClient (droplet_config.json) — correct IP 104.236.102.57 only.
Usage: python scripts/run_entry_intelligence_parity_audit_on_droplet.py [--date YYYY-MM-DD]
"""
...
def main() -> int:
    ...
    cmd = f"cd {REMOTE_ROOT} && git pull origin main && REPO_DIR={REMOTE_ROOT} bash scripts/run_entry_intelligence_parity_audit_on_droplet.sh{date_arg}"
    print(f"[RUN] {cmd}")
    ...
    report_name = f"STOCK_ENTRY_INTELLIGENCE_PARITY_AND_GAPS_{date_str}.md"
