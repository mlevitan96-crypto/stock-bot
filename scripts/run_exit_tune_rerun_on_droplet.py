#!/usr/bin/env python3
"""Run CURSOR_EXIT_SIGNAL_TUNE_AND_RERUN.sh on droplet via DropletClient."""
...
FILES = [
    "scripts/CURSOR_EXIT_SIGNAL_TUNE_AND_RERUN.sh",
    "scripts/CURSOR_HISTORICAL_EXIT_TRUTH_HARVEST_AND_REVIEW.sh",
    "scripts/analysis/extract_exit_tuning_directives.py",
    "scripts/analysis/apply_exit_signal_tuning.py",
    "scripts/analysis/compare_exit_edge_runs.py",
...
def main() -> int:
    ...
    with DropletClient() as c:
        ...
        for sh in ["CURSOR_EXIT_SIGNAL_TUNE_AND_RERUN.sh", "CURSOR_HISTORICAL_EXIT_TRUTH_HARVEST_AND_REVIEW.sh"]:
            c._execute(f"sed -i 's/\\r$//' {pd}/scripts/{sh} 2>/dev/null; chmod +x {pd}/scripts/{sh} 2>/dev/null", timeout=5)

        cmd = f"cd {c.project_dir} && REPO={c.project_dir} bash scripts/CURSOR_EXIT_SIGNAL_TUNE_AND_RERUN.sh"
        ...
        if rc == 0 and "RUN_DIR:" in out:
            try:
                ...
                if run_dir and run_dir.startswith("/"):
                    ...
                    comp_out, _, _ = c._execute(f"cat '{run_dir}/edge_comparison.json' 2>/dev/null || true", timeout=5)
                    local_dir = REPO / "reports" / "exit_review"
                    local_dir.mkdir(parents=True, exist_ok=True)
                    summary_md = local_dir / "LATEST_TUNE_RERUN_SUMMARY.md"
                    summary_md.write_text(
                        f"# Latest Exit Tune + Rerun (from droplet)\n\n"
                        f"**Run dir (droplet):** `{run_dir}`\n\n## Summary\n\n```\n{summary_out}\n```\n\n"
                        f"## edge_comparison.json\n\n```json\n{comp_out}\n```\n",
                        encoding="utf-8",
                    )
