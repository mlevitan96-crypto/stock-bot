#!/usr/bin/env python3
"""
Run CURSOR_DROPLET_WIN_FINDING.sh on the droplet.
MIN_TRADES=2000; find winning entry+hold combinations on real data.
"""
...
FILES = [
    "scripts/CURSOR_DROPLET_WIN_FINDING.sh",
    "scripts/learning/generate_win_finding_policies.py",
...
def main() -> int:
    ...
    min_trades = os.environ.get("MIN_TRADES", "2000")
    ...
    with DropletClient() as c:
        ...
        c._execute(f"chmod +x {pd}/scripts/CURSOR_DROPLET_WIN_FINDING.sh 2>/dev/null", timeout=5)

        env_parts = [f"MIN_TRADES={min_trades}", f"ITERATIONS={iterations}"]
        if truth:
        ...
        cmd = f"cd {c.project_dir} && {env_str} bash scripts/CURSOR_DROPLET_WIN_FINDING.sh"
        print("\n--- Running WIN FINDING on droplet (MIN_TRADES=2000) ---")
        if truth:
            print(f"   TRUTH_30D_PATH={truth}")
        if intel:
            print(f"   INTEL_PATH={intel}")
        print(f"   MIN_TRADES={min_trades} ITERATIONS={iterations}\n")
        ...
        list_out, _, _ = c._execute(
            f"cd {c.project_dir} && ls -1dt reports/win_finding/win_finding_* 2>/dev/null | head -1",
            timeout=10,
        )
        run_tag = (list_out or "").strip()
        if run_tag:
            name = Path(run_tag).name
            out_dir = REPO / "reports" / "win_finding" / name
            ...
            print("No win_finding run dir found.", file=sys.stderr)
