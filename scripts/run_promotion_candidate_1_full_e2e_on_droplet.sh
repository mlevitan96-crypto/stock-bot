#!/usr/bin/env bash
# CURSOR TASK — Full promotion candidate E2E, persona extraction, append to promotion candidates, and concise persona summary
# Run on the droplet at /root/stock-bot as the repo user.
# Launches the E2E wrapper and tail+poll wrapper, then waits and runs persona extraction/append/print.
set -euo pipefail
...
if pgrep -f "run_promotion_candidate_1_e2e_on_droplet.sh" >/dev/null 2>&1; then
  echo "E2E wrapper already running; skipping start."
else
  echo "Starting E2E wrapper (background). Log: ${LOG}"
  nohup bash scripts/run_promotion_candidate_1_e2e_on_droplet.sh >> "${LOG}" 2>&1 &
  E2E_PID=$!
  echo "E2E PID ${E2E_PID}"
fi

# 2) Start tail+poll wrapper if not already running (safe; it will not duplicate artifacts)
...
# 3) Wait for key artifacts (persona JSON or board verdict, or metrics + PROMOTION_CANDIDATES + exec_sensitivity/board)
WAIT_MIN=60
SLEEP_SEC=10
...
while [ $i -lt $MAX_ITER ]; do
  ...
  if ! pgrep -f "run_promotion_candidate_1_e2e_on_droplet.sh" >/dev/null 2>&1; then
    echo "E2E wrapper not running; waiting 120s for final artifacts..."
    sleep 120
    break
  fi
  sleep "${SLEEP_SEC}"
...
# 4) If persona JSON missing, run persona extraction helper
