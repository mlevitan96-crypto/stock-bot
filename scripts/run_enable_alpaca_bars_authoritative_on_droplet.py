#!/usr/bin/env python3
"""
Run enable_alpaca_bars_authoritative.sh on the droplet via SSH.
Uses instance_a/.env, correct data host, hard fails. Prints full output.
"""
...
ROOT = "/root/stock-bot"
SCRIPT_NAME = "enable_alpaca_bars_authoritative.sh"
BARS_DEPLOY = [
...
def main() -> int:
    ...
    with DropletClient() as c:
        ...
        try:
            ...
            safe_print("Deployed authoritative bars script + dependencies.\n")
        finally:
            sftp.close()
        # Ensure parquet deps on droplet (Phase 3)
        ...
        verdict, _, _ = c._execute(f"cat {ROOT}/reports/bars/final_verdict.txt 2>/dev/null; ls -la {ROOT}/reports/bars/ 2>/dev/null", timeout=10)
        if (verdict or "").strip():
