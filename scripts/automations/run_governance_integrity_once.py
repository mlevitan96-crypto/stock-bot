...
# memory_bank/ may be a directory OR MEMORY_BANK.md at repo root — both are valid.
...
def check_repo_structure() -> tuple[str, list[str]]:
    ...
    if not any((REPO_ROOT / alt).exists() for alt in MEMORY_BANK_ALTERNATIVES):
        return "fail", ["Missing memory_bank/ dir and MEMORY_BANK.md file"]
    return "pass", details
...
def check_governance_contracts() -> tuple[str, list[str]]:
    ...
    details = []
    if not any((REPO_ROOT / alt).exists() for alt in MEMORY_BANK_ALTERNATIVES):
        details.append("Missing: memory_bank/ dir and MEMORY_BANK.md")
