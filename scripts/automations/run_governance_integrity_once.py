...
def check_repo_structure() -> tuple[str, list[str]]:
    ...
    # MEMORY_BANK.md at root is the canonical memory bank (no memory_bank/ dir)
    if not (REPO_ROOT / "MEMORY_BANK.md").is_file():
        return "fail", ["MEMORY_BANK.md missing at repo root"]
    return "pass", details
...
def check_governance_contracts() -> tuple[str, list[str]]:
    """.cursor/automations, reports/audit, reports/board, and MEMORY_BANK.md."""
    details = []
    ...
    if not (REPO_ROOT / "MEMORY_BANK.md").is_file():
        details.append("Missing: MEMORY_BANK.md (canonical memory bank)")
    if details:
        return "fail", details
    return "pass", details
