...
def check_repo_structure() -> tuple[str, list[str]]:
    ...
    has_memory_bank = bool(
        ("memory_bank" in top_dirs) or
        any(alt in (top_dirs | top_files) for alt in MEMORY_BANK_ALTERNATIVES)
    )
...
def check_governance_contracts() -> tuple[str, list[str]]:
    ...
    has_memory_bank = (
        (REPO_ROOT / "memory_bank").exists() or
        (REPO_ROOT / "MEMORY_BANK.md").exists()
    )
    if not has_memory_bank:
        details.append("Missing: memory_bank/ directory or MEMORY_BANK.md file")
    if details:
        return "fail", details
    return "pass", details
