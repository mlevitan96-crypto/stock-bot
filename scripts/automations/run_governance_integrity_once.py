...
def check_repo_structure() -> tuple[str, list[str]]:
    ...
    if not (REPO_ROOT / "memory_bank").is_dir() and not (REPO_ROOT / "MEMORY_BANK.md").is_file():
...
def check_governance_contracts() -> tuple[str, list[str]]:
    ...
    if not (REPO_ROOT / "memory_bank").exists() and not (REPO_ROOT / "MEMORY_BANK.md").is_file():
