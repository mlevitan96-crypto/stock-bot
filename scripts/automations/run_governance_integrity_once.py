...
# memory_bank/ not required as a directory; repo uses MEMORY_BANK.md at root
...
def check_repo_structure() -> tuple[str, list[str]]:
    ...
    if not any((REPO_ROOT / alt).exists() for alt in MEMORY_BANK_ALTERNATIVES):
        return "fail", ["Missing memory_bank/ directory or MEMORY_BANK.md"]
    for sub in ["reports/audit", "reports/board"]:
        if not (REPO_ROOT / sub.replace("/", os.sep)).is_dir():
            details.append(f"Missing subdirectory: {sub}")
    if details:
        return "fail", details
    return "pass", details
