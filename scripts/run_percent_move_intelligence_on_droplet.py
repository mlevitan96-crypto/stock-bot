#!/usr/bin/env python3
"""
Run CURSOR_DROPLET_PERCENT_MOVE_INTELLIGENCE.sh on the droplet with real data.
Uploads script + analysis deps, runs, fetches report.
"""
...
FILES = [
    "scripts/CURSOR_DROPLET_PERCENT_MOVE_INTELLIGENCE.sh",
...
def main() -> int:
    ...
    move_pcts = os.environ.get("MOVE_PCTS", "0.5,1.0,1.5,2.0,3.0")
    ...
    with DropletClient() as c:
        ...
        c._execute(f"chmod +x {pd}/scripts/CURSOR_DROPLET_PERCENT_MOVE_INTELLIGENCE.sh 2>/dev/null", timeout=5)

        env_parts = [f"MOVE_PCTS={move_pcts}"]
        if truth:
            env_parts.append(f"TRUTH_30D_PATH={truth}")
        env_str = " ".join(env_parts)

        cmd = f"cd {c.project_dir} && {env_str} bash scripts/CURSOR_DROPLET_PERCENT_MOVE_INTELLIGENCE.sh"
        print("\n--- Running PERCENT MOVE INTELLIGENCE on droplet ---")
        if truth:
            print(f"   TRUTH_30D_PATH={truth}")
        print(f"   MOVE_PCTS={move_pcts}\n")
        ...
        list_out, _, _ = c._execute(
            f"cd {c.project_dir} && ls -1dt reports/percent_move_intelligence/percent_move_intelligence_* 2>/dev/null | head -1",
            timeout=10,
        )
        run_tag = (list_out or "").strip()
        if run_tag:
            name = Path(run_tag).name
            out_dir = REPO / "reports" / "percent_move_intelligence" / name
            ...
            print("No percent_move_intelligence run dir found.", file=sys.stderr)
