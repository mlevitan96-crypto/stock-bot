#!/usr/bin/env python3
"""
Vanguard V3 Alpha Hunter (shadow) — XGBoost targeting runners: exit_mfe_pct > 1.5%.

Loads ``reports/Alpaca/alpaca_ml_cohort_unified.csv``, label ``is_runner = (exit_mfe_pct > 0.015)``,
holdout top 30% by ``predict_proba`` positive class (no 0.5 threshold).
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-model",
        type=Path,
        default=REPO_ROOT / "models" / "vanguard_v3_hunter_agent.json",
    )
    ap.add_argument(
        "--out-meta",
        type=Path,
        default=REPO_ROOT / "models" / "vanguard_v3_hunter_threshold.json",
    )
    ap.add_argument(
        "--out-features",
        type=Path,
        default=REPO_ROOT / "models" / "vanguard_v3_hunter_features.json",
    )
    ...
    raw = raw.loc[raw["exit_mfe_pct"].notna()].copy()
    n = len(raw)
    if n < 50:
        print("Not enough rows after exit_mfe_pct filter.", file=sys.stderr)
        return 1
    ...
    mfe = pd.to_numeric(raw["exit_mfe_pct"], errors="coerce")
    raw["is_runner"] = (mfe > 0.015).astype(np.int32)
    ...
    order_idx = np.argsort(-proba)
    thr = float(proba[order_idx[k - 1]])
    approved_mask = proba >= thr
    n_ap = int(approved_mask.sum())
    runner_rate = float(y_test[approved_mask].mean()) if n_ap else float("nan")

    imp = clf.feature_importances_
    order_imp = np.argsort(-imp)
    top10 = [(feat_names[i], float(imp[i])) for i in order_imp[:10]]

    print("=== Vanguard V3 Alpha Hunter (top-%s holdout slice) ===" % (int(top_frac * 100),))
    print("rows (exit_mfe_pct not null):", n)
    print("train:", len(X_train), "test:", len(X_test))
    print("pre_decision_features_n:", len(feat_names))
    print("positive rate (runners) full:", float(raw["is_runner"].mean()))
    print("Test Accuracy (default 0.5 predict):", round(acc, 4))
    print("Top-30% probability threshold (inclusive):", round(thr, 6))
    print("Approved count on holdout:", n_ap, "of", n_test)
    print("Runner rate on approved holdout slice:", round(runner_rate, 4) if np.isfinite(runner_rate) else "n/a")
    ...
    thr_payload = {
        ...
        "holdout_runner_rate_among_approved": runner_rate,
