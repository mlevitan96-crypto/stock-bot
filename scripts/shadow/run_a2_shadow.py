#!/usr/bin/env python3
"""A2 shadow: relax max_positions_reached. Read-only; writes state/shadow/A2_shadow.json and reports/audit/A2_SHADOW_RESULTS.md."""
...
def main() -> int:
    ...
    payload = {
        "shadow_id": "A2_shadow",
        "name": "Relax max_positions_reached",
        "run_ts": datetime.now(timezone.utc).isoformat(),
        "would_admit_count": n,
        "proxy_pnl_delta": round(n * avg, 2),
        "proxy_pnl_delta_label": "proxy",
        "win_rate_delta": None,
        "tail_risk_notes": ["Increases concentration; regulatory/risk limit exposure."],
        "cohort": "last-387",
    }
    (base / "state" / "shadow").mkdir(parents=True, exist_ok=True)
    (base / "state" / "shadow" / "A2_shadow.json").write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    (base / "reports" / "audit").mkdir(parents=True, exist_ok=True)
    (base / "reports" / "audit" / "A2_SHADOW_RESULTS.md").write_text(
