#!/usr/bin/env bash
# One-shot Cursor run: full automated pipeline + summary + report
# Usage example:
#   MODE=parallel WORKERS="worker1 worker2" GH_TOKEN="..." MAX_ITER=3 MIN_TRADES=100 bash cursor_one_shot_runall.sh
set -euo pipefail
...
# Mitigations and trials (tweak if you want different candidates)
MITIGATIONS=(
...
log "Starting one-shot Cursor full run (MODE=${MODE})"

# Launch orchestrator or focused check
ORCH_PID=""
case "${MODE}" in
  local)
...
while [ "${DECISION}" != "PROMOTE" ] && [ "${ITER}" -lt "${MAX_ITER}" ]; do
  ...
  if [ -z "${MIT}" ]; then log "No more mitigations"; break; fi
  ITER=$((ITER+1))
...
# Ensemble trials if still not PROMOTE
...
# Exhaustive slippage grid and ablation sweeps
...
log "Cursor one-shot run complete. Excerpts: ${TMP_DIR}. Log: ${LOG}"
exit 0
