#!/usr/bin/env python3
"""
Fix clusters UnboundLocalError and freeze state
"""
...
def main():
    ...
    try:
        ...
        # 3. Test check_freeze_state
        print("3. TESTING check_freeze_state()")
        print("-" * 80)
        result = client.execute_command(
            ...
            "    print(f'governor_freezes.json exists: {freezes}')\n"
            "else:\n"
            "    print('governor_freezes.json does NOT exist')\n"
            "pre_freeze = Path('state/pre_market_freeze.flag')\n"
            "if pre_freeze.exists():\n"
            "    print(f'pre_market_freeze.flag exists: {pre_freeze.read_text()[:100]}')\n"
            "else:\n"
            "    print('pre_market_freeze.flag does NOT exist')\n"
            "PYEOF",
            timeout=60
        )
        print(result['stdout'])
        ...
        # 7. Check for errors
        print("7. CHECKING FOR ERRORS")
        print("-" * 80)
        result = client.execute_command(
            "journalctl -u trading-bot.service --since '2 minutes ago' --no-pager 2>&1 | grep -E 'UnboundLocalError|EXCEPTION|ERROR|halted_freeze' | tail -20",
            timeout=10
        )
