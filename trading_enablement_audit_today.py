...
def main() -> int:
    with DropletClient() as c:
        ...
        cycles = [e for e in run if e.get("msg") in ("complete", "cycle_complete")]
        cycles = cycles[-20:]
        ...
        for cy in cycles:
            ...
            if not dt:
                continue
            w_scoring = _window(scoring_flow, dt)
            w_gate = _window(gate, dt)
            w_orders = _window(orders, dt)
            w_exit = _window(exits, dt)
        ...
        gate_msgs = [str(e.get("msg")) for e in gate if e.get("msg")]
        top_gate = Counter(gate_msgs).most_common(30)

        run_once_msgs = [str(e.get("msg")) for e in run_once if e.get("msg")]
        top_run_once = Counter(run_once_msgs).most_common(30)

        run_halts = [e for e in run if e.get("msg") in ("halted_freeze",)]

        # Spot common "no entries" states
        capacity_blocks = [e for e in gate if e.get("msg") in ("max_positions_reached", "max_new_positions_per_cycle_reached")]
        score_blocks = [e for e in gate if e.get("msg") in ("score_below_min", "expectancy_blocked")]
        cooldown_blocks = [e for e in gate if e.get("msg") in ("symbol_on_cooldown",)]

        # Stale bars signals
        stale_bars = [e for e in market_data if e.get("msg") == "stale_bars_detected"]

        report = {
            "droplet": {"now_utc": now_utc, "utc_day": utc_day, "git_head": head, "service_active_enter": active_enter},
            "counts_today": {
                "run": len(run),
                "run_once": len(run_once),
                "scoring_flow": len(scoring_flow),
                "gate": len(gate),
                "orders": len(orders),
                "exit": len(exits),
