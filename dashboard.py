...
<body>
    ...
    window.loadDirectionBanner=function(){fetch('/api/direction_banner',{credentials:'same-origin'}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}).then(function(d){var el=document.getElementById('direction-banner');if(!el)return;var sev=(d&&d.severity)||'info';el.className='direction-banner '+sev;if(!d){el.innerHTML='Direction status unavailable';return;}var msg=d.message||'';var detail=d.detail||'';var link=d.link;var html=msg;if(detail){html+=' <span style="opacity:0.9;">'+detail+'</span>';}if(link){html+=' <a href="'+link+'" target="_blank" rel="noopener">View report</a>';}el.innerHTML=html;});};
