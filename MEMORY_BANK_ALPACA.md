...
- **wheel (V3 Options — cash-collateralized CSP / CC when enabled)** — **Circular execution:** `src/wheel_manager.run_wheel` calls **`reconcile_assignments_from_broker`** (`src/wheel_manager.py`) before each cycle. That path uses **`api.list_positions()`**: for each open CSP leg in `wheel_state`, if the **OCC short symbol is no longer in positions** and the underlying shows **≥100 shares** (multiple of 100), the leg is moved to **`assigned_shares`** (assignment) with telemetry **`wheel_assignment_detected`**; cleared legs without stock emit **`wheel_csp_leg_cleared`**. **`_run_cc_phase`** then sells covered calls against **`assigned_shares`** using broker quotes — **no manual step** when broker and state align. **Mid-cap universe (2026-05-04):** canonical list **`config/wheel_universe.yaml`** (wired via **`strategies.wheel.universe_source`**); **Deep Bench** expansion targets **45+** liquid mid-cap names (~**$10–$60** spot band) with sector tags in **`strategies/wheel_universe_selector.SECTOR_MAP`** so **`sector_cap_allows_new_csp`** (25% of equity per sector bucket) remains the hard ceiling. **`strategies.wheel_universe_selector.py`** (PATH B) honors **`universe_excluded_sectors`** as an explicit list (**empty `[]` is allowed** and does not fall back to defaults). Capital pre-filter and **`per_position_fraction_of_wheel_budget`** (~**0.52** on main config) set **`max_affordable_share_price`** so **~$10–$50** names can pass at **~$46k equity** while **sum of CSP notionals** remains within the wheel slice (**`can_allocate`**). **Premium ledger:** `wheel_state.json` field **`wheel_premium_by_ticker`** accumulates **filled** CSP + CC premium per underlying; **`wheel_premium_cumulative`** events and **`INFO` `Wheel premium cumulative …`** lines are emitted on each fill; **`src/wheel_dashboard_sink.build_wheel_hud_rows`** exposes **`realized_premium_usd`** from that map when present. **`risk.csp_max_abs_estimated_delta`** (default **0.5**) rejects CSP candidates whose **|estimated put delta|** exceeds the cap (stricter than the delta band alone). **Duplicate orders:** CSP checks **`list_orders(status="open")`** for an existing row on the **option OCC** before submit; CC does the same for the **call OCC**. Options path: `strategies/wheel_strategy.py`, **`src/options_engine.py`**, **`src/wheel_manager.py`**. **Execution:** `AlpacaExecutor._submit_order_guarded` for wheel submits. **Pager:** see **§2.2.1a**. Wheel premium milestones: `scripts/wheel_premium_milestone_watcher.py`.

## 2.2.1a BOARD SRE — CRITICAL ALERTING & WHEEL STATE RECOVERY (2026-05-04)
- **Wheel First-5 submit integrity pager (2026-05-04):** On each successful Wheel broker submit (**CSP** and **CC** paths in `strategies/wheel_strategy.py`), **`src/wheel_first_five_telegram.maybe_telegram_wheel_first_five_submit`** may send one Telegram via **`scripts/alpaca_telegram.send_governance_telegram`** with **`script_name=wheel_first_five_submit`** (allowlisted under **`TELEGRAM_GOVERNANCE_INTEGRITY_ONLY=1`**). At most **five** messages for the life of the checkout; counter and **`order_id`** dedupe persist in **`state/wheel_first_five_submit_state.json`** (**`StateFiles.WHEEL_FIRST_FIVE_SUBMIT`**). Payload includes underlying, action, strike, **[IV Rank]**, **[Premium USD]** (expected credit = limit × 100 at submit), **[Underlying mid]**, **[Put wall strike]** (CSP only; CC uses `n/a` for wall). Disable with **`WHEEL_FIRST_FIVE_TELEGRAM=0`**.
