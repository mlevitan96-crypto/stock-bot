...
- **WebSocket auth (operational):** Default **`UW_WS_AUTH_MODE=bearer`** uses a clean `wss://…/socket` URL and **`Authorization: Bearer <UW_API_KEY>`** on the upgrade (URL `?token=` alone may 401). Set **`UW_WS_AUTH_MODE=query`** or **`both`** if UW requires legacy dual auth. UW still returns **HTTP 401** if the key lacks **WebSocket / `flow-alerts` entitlement**; the daemon **keeps running** and retries with backoff; REST + cache remain active. Healthy subscribe prints **`[UW-WS] flow-alerts join ok (server ack)`** after the server `status: ok` frame. Remediation: upgrade UW API plan or set **`UW_FLOW_WS_ENABLED=0`** until the key is entitled.
