...
- **Hard REST daily cap (Alpaca strike):** **`src/uw/uw_client.py`** enforces **`UW_DAILY_LIMIT` × `UW_SAFETY_BUFFER`** against `state/uw_usage_state.json` (defaults **`UW_DAILY_LIMIT=50000`**, **`UW_SAFETY_BUFFER=0.95`** → **47,500** effective calls/UTC day unless env overrides). Same defaults in **`config/registry.py`** for dashboards/scripts. **`uw_flow_daemon`** logs the effective cap at startup and **lengthens** **`UW_DAEMON_MIN_LOOP_SLEEP_SEC`** when local usage ratio **≥ 80%** (self-throttle on top of RTH-only market-sensitive polling 9:30–16:00 ET).
