...
- **WebSocket auth (operational):** UW returns **HTTP 401** on the socket if the API key lacks **WebSocket / `flow-alerts` scope** (per UW pricing/docs). The daemon **keeps running** and retries with backoff; REST + cache remain active. Remediation: upgrade UW API plan or set **`UW_FLOW_WS_ENABLED=0`** until the key is entitled.
