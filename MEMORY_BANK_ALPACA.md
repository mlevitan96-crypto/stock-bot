...
# Version: 2026-05-03 (LIVE — V2 bidirectional gate; V3 wheel upstream capital universe filter + SRE pager + state heal; MEMORY_BANK.md index; fail-closed wheel broker reads)
# Last Updated: 2026-05-03 (wheel universe upstream capital pre-filter; §2.2.1a Board SRE; see **MEMORY_BANK.md** for short index)
...
- **State (2026-04-28 UTC):** **LIVE / ACTIVE HUNTING** — Alpaca production **`stock-bot.service`** loads current **`main`** + **`telemetry/shadow_evaluator.py`** + **`telemetry/vanguard_ml_runtime.py`** (Split-Brain Shadow Vanguard: challenger long/short models, **`logs/shadow_executions.jsonl`** telemetry only; no shadow rows in broker ledger). **V2 live gate is bidirectional** (short quarantine lifted — §1.0.2). Harvester-era entry snapshots, strict epoch, and live gates (V2 profit agent long+short, Alpha 10 env, etc.) remain as documented in §1.0 / §1.0.2–1.0.4.
...
- **V2 live gate — bidirectional (lifted 2026-04-28 UTC):** The **V2 Short Quarantine is removed.** Short/sell **entries** are evaluated by the **same live Vanguard V2 Profit Agent** path as long/buy: `telemetry/vanguard_ml_runtime.py` `evaluate_v2_live_gate` loads the primary V2 model, applies side-normalized features, and applies the **same probability threshold** (plus `V2_LIVE_GATE_ENABLED` / `V2_LIVE_GATE_FAIL_OPEN` semantics for both sides). The legacy telemetry reason **`v2_short_gate_quarantined_until_retrain`** is **retired** (no longer emitted). **Broker constraints unchanged:** Alpaca short entries still require **`shortable` AND `easy_to_borrow`** in `submit_entry` unless **`HTB_OVERRIDE`** is explicitly enabled (`src/core/broker_math.py` `validate_short_asset`).
...
- **Broker pull (account truth):** Uses **`alpaca_trade_api.REST.get_account()`** with standard env keys (`ALPACA_KEY`/`ALPACA_SECRET`/`ALPACA_BASE_URL`, etc.). Persisted fields include at minimum **`equity`**, **`buying_power`**, **`multiplier`**, plus read-only status flags (`trading_blocked`, margin fields, …). **Deprecated (removed from snapshot JSON, per Alpaca FINRA 4210 API cleanup):** `pattern_day_trader`, `daytrade_count`, `daytrading_buying_power`. Output: **`state/alpaca_account_snapshot.json`** (gitignored path on clones; always written on the droplet run).
...
- **equity (Alpaca V2 Harvester — LEGACY / OPTIONAL when wheel-only lock)** — UW-driven equity strategy. `strategies/equity_strategy.py`. Live/paper telemetry uses `strategy_id=equity` where recorded. **Many droplets have moved to wheel-only or reduced equity surface**; treat equity-first prose elsewhere in this file as **historical unless `strategies.equity.enabled` is true** on that host.
- **wheel (V3 Options — cash-collateralized CSP / CC when enabled)** — Options wheel path: `strategies/wheel_strategy.py` (CSP/CC + `state/wheel_state.json`), **`src/options_engine.py`** (S&P 100 hard gate, UW IV rank + earnings + oi-change put wall + dust-floor helper), **`src/wheel_manager.py`** (assignment reconciliation + `run_wheel` entry). Universe YAML under `config/universe_wheel*.yaml`, toggles in `config/strategies.yaml`. **`strategies/wheel_universe_selector.py`** (PATH B) calls **`get_account()`** once per selection pass and loads **`risk.max_sector_notional_fraction`**, wheel budget via **`capital.strategy_allocator.can_allocate`**, **`state/wheel_state.json`** `open_csps`, and **`per_position_fraction_of_wheel_budget`**. Before any per-symbol **bars / options contracts / spread** work, it resolves a conservative **`max_affordable_share_price` = min(sector headroom, wheel available, per-position limit, [cash/0.99] when strict cash applies)** where **sector headroom** is `(max_sector_fraction × equity − current sector CSP notional)` and **affordable share price** is that headroom divided by **100** (one CSP contract ≈ 100 shares × strike; strike ≈ spot for the screen). Symbols with **spot above that floor** are **dropped upstream** with an INFO log (`Wheel universe: capital pre-filter drop …`). If **`get_account()`** fails, the capital pre-filter is skipped (fail-open) so the wheel still runs. **Execution:** `main.run_all_strategies` builds one `tradeapi.REST(..., api_version='v2')` wrapped in **`AlpacaExecutor(external_api=...)`** so wheel orders use **`_submit_order_guarded`** (audit guard, paper safety, `submit_order_called` truth). **Quotes:** `fetch_alpaca_latest_quote` + `normalize_alpaca_quote` (no legacy `get_quote` on REST). **Broker reads:** `list_positions` / `list_orders` failures **fail-closed** (cycle abort, no empty-book assumption). **State:** malformed `wheel_state.json` is **self-healed** with `state/wheel_state.corrupt.<ts>.json` backup — never invent broker legs; **`reconcile_assignments_from_broker`** remains authoritative. **Pager:** `src/critical_alert.py` + `scripts/alpaca_telegram.py` (`script_name=alpaca_wheel_critical`, integrity-only allowlist). See **§2.2.1a**. Wheel premium milestones: `scripts/wheel_premium_milestone_watcher.py`.

## 2.2.1a BOARD SRE — CRITICAL ALERTING & WHEEL STATE RECOVERY (2026-05-04)
- **Wheel First-5 submit integrity pager (2026-05-04):** On each successful Wheel broker submit (**CSP** and **CC** paths in `strategies/wheel_strategy.py`), **`src/wheel_first_five_telegram.maybe_telegram_wheel_first_five_submit`** may send one Telegram via **`scripts/alpaca_telegram.send_governance_telegram`** with **`script_name=wheel_first_five_submit`** (allowlisted under **`TELEGRAM_GOVERNANCE_INTEGRITY_ONLY=1`**). At most **five** messages for the life of the checkout; counter and **`order_id`** dedupe persist in **`state/wheel_first_five_submit_state.json`** (**`StateFiles.WHEEL_FIRST_FIVE_SUBMIT`**). Payload includes underlying, action, strike, **[IV Rank]** (UW `fetch_iv_rank` at submit time), **[Underlying mid]** (spot at submit), **[Put wall strike]** (CSP only; CC uses `n/a`). Disable with **`WHEEL_FIRST_FIVE_TELEGRAM=0`**.
- **Vane (uptime / fail-closed):** Critical Telegram uses existing **`TELEGRAM_BOT_TOKEN`** / **`TELEGRAM_CHAT_ID`** (same as governance). Env: **`CRITICAL_ALERTS_ENABLED`** (default on when tokens set), **`CRITICAL_ALERT_COOLDOWN_SEC`** (default **600** per dedupe key in `state/critical_alert_dedupe.json`), **`CRITICAL_ALERT_BYPASS_MARKET_HOURS=1`** to send overnight when governance quiet-hours would suppress (use sparingly).
- **Marcus (spam / abuse):** One dedupe bucket per `dedupe_key` (e.g. `wheel_abort:list_positions`); cooldown prevents tight loops from paging every tick. Integrity-only droplets must allow **`alpaca_wheel_critical`** in `scripts/alpaca_telegram.py` allowlist or sends are blocked by design.
- **Sterling (capital math):** State heal **drops** non-dict leg rows and coerces containers; it does **not** synthesize strikes or quantities. Unreadable JSON → safe defaults + backup file; operator reconciles broker vs disk if needed.
...
- **Ingestion spine:** `uw_flow_daemon.py` is the single writer to `data/uw_flow_cache.json`. With **`UW_REST_BUDGET_MODE=1`**, the daemon loads tier lists from the daily universe files (else partitions `TICKERS`), uses **longer REST intervals** for structural endpoints, and optionally starts **`uw_flow_ws.py`** (async `websockets`) against **`wss://api.unusualwhales.com/socket`**, subscribing to **`flow-alerts`**, filtering to **Sniper** symbols only. **`UW_FLOW_WS_ENABLED=1`** in `.env` enables WS (default **on** when budget mode is on); set **`0`** to force REST-only flow for Snipers. After rotating **`UW_API_KEY`**, sanitize `.env` (no trailing whitespace / stray quotes) and run **`scripts/debug/ws_smoke_test.py`** until it prints **`SUCCESS: JOIN OK`**; droplet helper **`scripts/debug/droplet_env_sanitize_and_ws_verify.py`** (optional **`DROPLET_HOST=104.236.102.57`**) automates sed/dequote, forces **`UW_FLOW_WS_ENABLED=1`**, runs smoke, then **`git pull`** + **`systemctl restart uw-flow-daemon`**.
...
- **Hard REST daily cap (Monthly plan / strict):** **`UW_DAILY_LIMIT` must remain `50000`** (do not raise to 80k). **`src/uw/uw_client.py`** enforces **`UW_DAILY_LIMIT` × `UW_SAFETY_BUFFER`** against `state/uw_usage_state.json` (defaults **`UW_DAILY_LIMIT=50000`**, **`UW_SAFETY_BUFFER=0.95`** → **47,500** effective REST calls/UTC day unless env overrides). Same defaults in **`config/registry.py`** for dashboards/scripts. **`uw_flow_daemon`** logs the effective cap at startup and **lengthens** **`UW_DAEMON_MIN_LOOP_SLEEP_SEC`** when local usage ratio **≥ 80%** (self-throttle on top of RTH-only market-sensitive polling 9:30–16:00 ET).
- **REST quota circuit breaker (daemon):** At the start of each **`_poll_ticker`** call, if **`uw_daily_usage_ratio() >= 0.92`** (same 50k-based ratio as `uw_client`), the daemon logs **`WARNING: REST Quota Circuit Breaker Tripped - Sweeps Aborted`**, sets an internal flag, **returns** from that ticker, and the main loop **skips remaining tickers** for the rest of that cycle (flag cleared at the next cycle). WebSocket ingestion is unchanged; this only stops further REST sweeps for the current pass.
